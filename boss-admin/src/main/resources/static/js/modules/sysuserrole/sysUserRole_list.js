...
layui.use(['form','layer','table','laytpl','laydate'],function(){
    ...
    var tableIns = table.render({
        elem: '#sysUserRoleList',
        url : contextPath + '/sysUserRole/listSysUserRoles',
        cellMinWidth : 150,
        page : true,
        /*height : "full-200",*/
        limits : [10,15,20,25],
        limit : 15,
        id : 'sysUserRoleListTable',
        method : 'get',
        request : {
        ...
        cols : [[
            ...
            {title: '操作', width:220, fixed:"right", templet:'#sysUserRoleListBar',align:"center"}
        ]]
    });
    ...
    form.on('submit(searchBtn)', function(data){
        var index = layer.load(1);
        //重新渲染数据表格
        table.reload("sysUserRoleListTable",{
            page: {
    ...
    //toolbar事件
    table.on('tool(sysUserRoleList)',function (obj) {
        var event = obj.event;
        var data = obj.data;
        if(event === 'edit'){//修改按钮
            layerBox({
                title : '修改用户角色表',
                url : contextPath + "/sysUserRole/toEditSysUserRole?id="+data.id
            });
            ...
            layer.confirm('你确定删除吗?', {icon: 3, title:'提示'}, function(index){
                ...
                $.ajax({
                    type : 'post',
                    url : contextPath + '/sysUserRole/remove',
                    data : {
                        "id": data.id
                    },
                    dataType : 'json',
    ...
    $('#btn_add').on('click',function () {
        layerBox({
            title : '添加用户角色表',
            url : contextPath + "/sysUserRole/toAddSysUserRole"
        })
    });
    ...
    $('#btn_delete_all').on('click',function () {
        var datas = table.checkStatus('sysUserRoleListTable').data;
        if(datas.length <= 0){
            warnMsg('请先选择数据！');
        }else{
            layer.confirm('你确定删除选中的数据吗?', {icon: 3, title:'提示'}, function(index){
                ...
                $.ajax({
                    type : 'post',
                    url : contextPath + '/sysUserRole/batchRemove',
                    data : {
                        "ids[]": ids
                    },
                    dataType : 'json',
